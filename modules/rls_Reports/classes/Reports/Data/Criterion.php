...
class Criterion
{
    ...
    static public function getUserOffsetByTimezone()
    {
        ...
        $interval = $user_tz->getOffset($user_dt);
        $hour = intval($interval/3600);
        $minute = intval(($interval%3600)/60);
        $minute += $hour * 60;
        
        return 'INTERVAL '. $minute .' MINUTE';
    }
    ...
    private function setLeftJoinsFromFieldsSettings(array $settings) {
        ...
        foreach($settings as $field){
            ...
            if(array_key_exists('vardefs',$field)){
                ...
                if(array_key_exists('type',$vardef_cstm)
                    ...
                    //$module_of_rel_report = loadBean($field['module_of_report']);
                    $module_of_rel_report = loadBean($field['module_of_field']);
        ...
        $module_of_report_name = $report_settings['data']['module_of_report']; // RLS Vic #12817
        $module_of_report = loadbean($module_of_report_name);
        $alies_separator = '_'.$module_of_report->object_name.'_';
                ...
                if(
                    ...
                    if(
                        ...
                        if($field_def['id_name'] == 'currency_id' && array_key_exists('currency_id',$module->field_defs)){
                            if(
                                array_key_exists('source',$module->field_defs['currency_id'])
                                && ($module->field_defs['currency_id']['source'] == 'custom_fields'
                                || $module->field_defs['currency_id']['source'] == 'non-db')
                            ){
                                $table = $module->table_name.'_cstm';
                            } else {
                                $table = $module->table_name;
                                foreach ($this->globalAliases as $alias => $value) { // RLS Vic #12817
                                    $res = explode ($alies_separator, $alias);
                                    if (count($res) == 2 && $res[1] == $table) {
                                        $table = $alias;
                                        break;
                                    }
                                }
                    ...
                    $left_join = ' LEFT JOIN ' . $m_bean->table_name . ' as ' . $m_bean->table_name . '_for_relate_field' .
                                 ...
                                 ' AND ' . $m_bean->table_name . '_for_relate_field.deleted=0 ';
                    $joins_for_reletion_fields[$m_bean->table_name . '_for_relate_field'] = $left_join;
                    $this->additionalLeftJoinsForReletionField[$m_bean->table_name . '_for_relate_field'] = $left_join;
