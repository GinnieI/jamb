...
class Summarizing extends Operations
{
    ...
    public function getFunction($level = 0)
    {
        ...
        if(
            ...
            $function = parent::getFunction($level);
            if ($function == 'COUNT') 
                $function = $function. '(DISTINCT '. $this->getFieldname($level) .')'; // RLS Vic #12817
            else $function = $function. '('. $this->getFieldname($level) .')';
        }
...
//        if (parent::getFunction($level) == 'COUNT') { // RLS Vic commented #12817
//            return '*';
//        }
